...
<x-modal.modal-icon icon="bi-pencil-square" iconcolor="text-yellow-400" iconcolorhover="text-yellow-600">
    ...
    <form class="mt-4" action="{{route('user.brag.store')}}" method="POST">
        ...
        <div class="w-full md:w-1/4">
            ...
            <input type="date" name="date" value="{{$date}}" required class="block w-full px-4 py-2 mt-2 text-gray-700 bg-gray-100 border border-gray-100 rounded-md md: focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 focus:outline-none focus:ring focus:bg-white">
        </div>
        <div class="my-3">
            <label class="text-gray-700" for="brag">Capaian</label>
            <textarea name="brag" id="" rows="3" required class="block w-full px-4 py-2 mt-2 text-gray-700 bg-gray-100 border border-gray-100 rounded-md md: focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 focus:outline-none focus:ring focus:bg-white">{{$brag}}</textarea>
        </div>
